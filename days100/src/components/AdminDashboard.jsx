...
export default function AdminDashboard({ token, onLogout }) {
  ...
  return (
    <>
    <div className="min-h-screen !p-10 bg-bg text-text animate-fade-up">
      ...
      <div className="flex justify-between items-center border-b border-border !pb-6 !mb-8 ">
        <div>
          ...
          <p className="font-mono-jetbrains text-[0.6rem] uppercase tracking-[2px] text-muted">Database Records Manager</p>
        </div>
        <div className="flex gap-4">
          <button onClick={() => openModal()} className="font-mono-jetbrains text-[0.6rem] tracking-[1.5px] uppercase !px-4 !py-2.5 rounded-full border border-dsa text-dsa bg-dsa/3 transition-all duration-150 cursor-none hover:bg-dsa hover:text-black">+ Create Challenge</button>
          <button onClick={handleLogout} className="font-mono-jetbrains text-[0.6rem] tracking-[1.5px] uppercase !px-5 !py-2.5 rounded-full transition-all border border-devops text-devops bg-devops/3 duration-150 cursor-none hover:bg-devops hover:text-black">Exit Console</button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="w-full overflow-x-auto rounded-2xl border border-border bg-card">
          <table className="w-full text-left font-outfit text-sm">
            <thead className="font-mono-jetbrains text-[0.58rem] uppercase tracking-[1.5px] border-b border-border bg-[#090d18] text-muted">
              <tr>
            ...
            <tbody className="divide-y divide-border">
              {challenges.map((c) => (
                <tr key={c._id} className="hover:bg-border2/20 transition-colors duration-100">
                  ...
                  <td className="!py-3.5 !px-6 font-semibold text-text max-w-xs truncate">{c.title}</td>
                  <td className="!py-3.5 !px-6">
                    <span className="font-mono-jetbrains text-[0.55rem] px-2.5 py-0.5 rounded-md" style={{ 
                  ...
                  <td className="!py-3.5 !px-6 text-center">
                    <div className="flex justify-center gap-3 font-mono-jetbrains text-[0.58rem] tracking-[1px]">
                      ...
                      <span className="opacity-20 text-text">|</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm !mt-12 !p-4 animate-fade-up">
          <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl !p-6 border border border-border bg-card modal-box">
            <div className="flex justify-between items-center !mb-5">
              ...
              <button onClick={() => setModalOpen(false)} className="text-sm text-muted hover:text-text font-mono-jetbrains transition-colors cursor-none">✕ Close</button>
            </div>
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-4 font-outfit text-sm">
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input type="number" required value={form.day} onChange={e => setForm({ ...form, day: parseInt(e.target.value) || '' })} className="w-full rounded-xl !p-2.5 outline-none text-white bg-[#121829] border border-border2 text-text focus:border-dsa"/>
                </div>
                <div>
                  <label className="block text-[0.6rem] font-mono-jetbrains uppercase tracking-[1px] !mb-1 opacity-60 ">Tech Core Track</label>
                  <select value={form.tech} onChange={e => setForm({ ...form, tech: e.target.value })} className="w-full rounded-xl !p-2.5 outline-none bg-[#121829] border border-border2 text-text focus:border-dsa">
                    {TECH_TRACKS.map(t => <option key={t} value={t} className="bg-[#0c1120]">{t}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-[0.6rem] font-mono-jetbrains uppercase tracking-[1px] !mb-1 opacity-60">Difficulty Index</label>
                  <select value={form.diff} onChange={e => setForm({ ...form, diff: parseInt(e.target.value) })} className="w-full rounded-xl !p-2.5 outline-none bg-[#121829] border border-border2 text-text focus:border-dsa">
                    {Object.entries(LEVEL_MAP).map(([val, name]) => <option key={val} value={val} className="bg-[#0c1120]">{name}</option>)}
                  </select>
                </div>
              </div>
              ...
              <div>
                ...
                <input type="text" required value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="e.g., Implementing LRU Cache structure" className="w-full rounded-xl !p-2.5 outline-none bg-[#121829] border border-border2 text-text focus:border-dsa"/>
              </div>

              <div>
                <label className="block text-[0.6rem] font-mono-jetbrains uppercase tracking-[1px] !mb-1 opacity-60">Problem Statement</label>
                <textarea rows="4" value={form.pro_statement} onChange={e => setForm({ ...form, pro_statement: e.target.value })} placeholder="Provide system overview details here..." className="w-full rounded-xl !p-2.5 outline-none font-mono-jetbrains bg-[#121829] border border-border2 text-text text-xs focus:border-dsa"></textarea>
              </div>
              ...
              <div>
                ...
                <textarea rows="6" value={form.code} onChange={e => setForm({ ...form, code: e.target.value })} placeholder="// Paste clean solution functions here..." className="w-full rounded-xl !p-2.5 outline-none font-mono-jetbrains bg-[#121829] border border-border2 text-text text-xs focus:border-dsa"></textarea>
              </div>
              ...
              <div>
                ...
                className="w-full rounded-xl !p-2.5 outline-none  font-mono-jetbrains bg-[#121829] border border-border2 text-text text-xs focus:border-dsa"
                />
              </div>
